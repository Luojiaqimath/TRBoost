...
class TRBLearner:
    ...
    def __init__(self,
        ...
        self._n_estimators = n_estimators # boosting个数
        self._base_score = base_score # f_0的值，回归可设为0，分类设为类别为0.5
        self._max_depth = max_depth # 第一棵树的深度，即初始深度
        
        self._tree = None
        self._trees = [] # boosting tree集合
        self._epsilon = 1e-6 # 纠正误差
        self._min_child_weight = min_child_weight
        self._min_leaf = min_leaf #节点最小样本数
        self._min_nodenum = min_nodenum
        
        self._alpha = alpha # 半径1
        self._beta = beta  # 半径2
        self._rho_1 = rho1 # 信赖域判断条件1
        self._rho_2 = rho2 # 信赖域判断条件2
        self._gamma = gamma # 扩大比例
        self._eta = eta # 与近似率相比 是否添加树的阈值
    ...
    def fit(self, train_set, eval_set=None):
        ...
        initial_loss = self._objective.loss(labels_train, predictions)  # loss_0
        for i in range(self._n_estimators):
            # print('tree {} '.format(i+1)+'-'*10)
            
            # initialization
            prev_loss = self._objective.loss(labels_train, predictions) # loss_(k-1)
            ...
            tree.split(instances_train, labels_train, last_predictions=prev_pred)
            self._trees.append(tree)
            predictions = self.predict(instances_train) # y_k
            train_loss = self._objective.loss(labels_train, predictions) # loss_k
            ...
            if self._update_strategy == 'standard':          
                mk_residual = np.multiply(grad, pred_k) + 0.5 * np.multiply(hess, pred_k**2)
                rho_k = np.divide(prev_loss-train_loss, -np.mean(mk_residual))
                # print(mk_residual)
                # print(np.multiply(grad, pred_k))
                
            elif self._update_strategy == 'gradient':
                rho_k = np.divide(prev_loss-train_loss, np.mean(np.abs(pred_k)))
                
            else:
                rho_k = np.divide(prev_loss-train_loss, initial_loss-train_loss)
                
            self.rho_list.append(rho_k)

            # print('rho_k={}'.format(rho_k))
            ...
            if self._eta <rho_k:
                if eval_set is None:
                    ...
                    eval_metric = self._objective.metric(labels_eval, pred_eval)
                    self.eval_metric_list.append(eval_metric)
            else:
                self._trees.pop(-1)
                predictions = prev_pred
                # print(f'train_loss:{train_loss:.4f}, delete tree')
                if eval_set is not None:
                    if len(self._trees) !=0 :
                        self.eval_loss_list.append(eval_loss)  # 上一步的eval loss
                        self.eval_metric_list.append(eval_metric)  # 上一步的eval metric
                    else:
