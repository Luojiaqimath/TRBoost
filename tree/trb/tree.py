...
class Tree:
    ...
    def split(self, instances, labels, last_predictions):
        ...
        self._root = Node(self._max_depth, self._min_nodenum, 
                          self._min_child_weight, self._min_leaf)
        self._root.split(split_finder, instances, gradients, hessians, depth=0)
        curr_accnode = self._root.current_nodenum()
        self._root.reset_nodenum()
        global loop
        # print(f'current nodes：',curr_accnode)
        loop += 1
    ...
    def get_nodenum(self):
        count = self._get_dump(self._root, depth=0, end=[],index=1)
        return count
