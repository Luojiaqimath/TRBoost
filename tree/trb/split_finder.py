...
class SplitFinderVec(SplitFinder):
    ...
    def find_split(self, instances, gradients, hessians):
        ...
        hessian_sum_lt_new = np.array([h if h>0 else h+self._alpha for h in hessian_sum_lt])
        ...
        hessian_sum_ge_new = np.array([h if h>0 else h+self._alpha for h in hessian_sum_ge])
