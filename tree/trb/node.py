...
class Node:
    ########## 加入最小nodenum ###########
    def __init__(self, max_depth, min_nodenum, min_child_weight, min_leaf):
    ...
    #################################################################################
        ...
        #     (sum_hessians != 0 and np.abs(sum_hessians) < self._min_child_weight) or
        #         (sum_hessians == 0 and np.abs(sum_gradients) < self._min_child_weight)):
        if ((gradients.shape[0] < self._min_leaf) or
            (depth == self._max_depth) or
        ...
        if split_value is None:
            ...
            ###########################################################
            self.left_child = Node(self._max_depth, self._min_nodenum,
                                   self._min_child_weight, self._min_leaf)
            self.right_child = Node(self._max_depth, self._min_nodenum,
                                    self._min_child_weight, self._min_leaf)
            ###########################################################
            nodenum += 2
            ###########################################################
    ...
    #####################################################
    def current_nodenum(self):
        global nodenum
        return nodenum

    def reset_nodenum(self):
        global nodenum
        nodenum = 1
        return
    #####################################################
